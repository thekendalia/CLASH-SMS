...
def get_clan_current_war(clan_tag):
    """
    Fetches the current war details of the clan using its tag.
    """
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Error fetching war details: {response.status_code} - {response.text}")

def get_clan_members(clan_tag):
    """
    Fetches the members of the clan using its tag.
    """
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Error fetching clan members: {response.status_code} - {response.text}")